...
async function main() {
    ...
    const imageChunks: ImageSplitChunk[] = await splitter
        .cleanOutFiles()
        .split();
    ...
    if (config.snapPointLines) {
        console.log("snapPointLines");
        for (const snapPointLine of config.snapPointLines) {
            const x0 = snapPointLine.x0;
            const y0 = snapPointLine.y0;
            const x1 = snapPointLine.x1;
            const y1 = snapPointLine.y1;
            const count = snapPointLine.count;
            delete snapPointLine.x0;
            delete snapPointLine.y0;
            delete snapPointLine.x1;
            delete snapPointLine.y1;
            delete snapPointLine.count;
            if (
                x0 === undefined ||
                y0 === undefined ||
                x1 === undefined ||
                ...
                throw new Error("bad snapPointLine");
            }
            const dx = (x1 - x0) / (count - 1);
            const dy = (y1 - y0) / (count - 1);
            for (let i = 0; i < count; i++) {
                const snapPoint = JSON.parse(JSON.stringify(snapPointLine));
                snapPoint.X = x0 + dx * i;
                snapPoint.Y = y0 + dy * i;
                template.SnapPoints.push(snapPoint);
            }
        }
    }
    ...
    for (const chunk of imageChunks) {
        ...
        const x =
            (chunk.vTop + chunk.vHeight / 2) * config.height -
            config.height / 2;
        cubeTemplate.Offset = {
    ...
    if (config.script) {
        template.ScriptName = config.script;
    }
