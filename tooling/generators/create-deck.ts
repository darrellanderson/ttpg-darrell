 ...
 * - sharedBack : use for back
 * - sharedHidden : use for hidden
...
async function main() {
    ...
    // ------------------------------------
    const cardsheetHiddenRelativeToAssetsTextures =
        path.normalize(config.output) + ".hide.jpg";

    if (config.sharedHidden) {
        console.log("\n----- CREATING HIDDEN CARDSHEET -----\n");

        // Boring merge of a single image (resizes).
        await new MergeImage()
            .setSrcFilesRelativeToPrebuild([config.sharedHidden])
            .setDstFileRelativeToAssetsTextures(
                cardsheetHiddenRelativeToAssetsTextures
            )
            .setPreShrink(config.preshrink)
            .merge();
    }
    ...
    if (config.sharedHidden) {
        template.HiddenTexture = cardsheetHiddenRelativeToAssetsTextures;
        template.HiddenIndex = -2;
    }
